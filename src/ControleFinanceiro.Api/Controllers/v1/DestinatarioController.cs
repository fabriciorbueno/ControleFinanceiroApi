...
namespace ControleFinanceiro.Api.Controllers.v1
{
    ...
    public class DestinatarioController : Controller
    {
        ...
        public DestinatarioController(IGenericService service)
        {
            _service = service;
        }
        ...
        {
            var response = await _service.BuscarTodosAsync(TabelaEnum.Destinatario);
        ...
        {
            var response = await _service.InserirAsync(TabelaEnum.Destinatario, categoriaRequest);
