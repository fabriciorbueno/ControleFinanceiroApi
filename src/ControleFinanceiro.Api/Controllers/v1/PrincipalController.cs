...
namespace ControleFinanceiro.Api.Controllers.v1
{
    ...
    public class PrincipalController : Controller
    {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(IEnumerable<PrincipalResponse>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> BuscarTodos()
        {
            var response = await _service.BuscarTodosAsync();
        ...
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Inserir(PrincipalRequest request)
        {
            var response = await _service.InserirAsync(request);
